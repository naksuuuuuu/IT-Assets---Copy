<?php 
    ...
    if(isset($_POST["req_grp"])){
        ...
        $sql = "SELECT Req_Type_ID, Req_Type_Name FROM IT_ASSET_REQ_TYPE WHERE Req_Group_ID = :req_grp";
        ...
        $brand = $_POST["brand"];
        $sql = "SELECT Model_Code, Model FROM it_asset_model WHERE Brand_Code=:brand";
        $stmt = oci_parse(connection(), $sql);
        oci_bind_by_name($stmt,":brand",$brand);
        oci_execute($stmt);
