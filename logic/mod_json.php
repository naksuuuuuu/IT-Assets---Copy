...
if(isset($_POST['po_number'])){
    ...
    B.PO_ITEM, B.UNIT_PRICE, to_char(B.WARRANTY_START_DATE, 'YYYY-MM-DD') as WARRANTY_START_DATE, 
    B.WARRANTY_MONTH, to_char(B.WARRANTY_EXPIRE_DATE, 'YYYY-MM-DD') as WARRANTY_EXPIRE_DATE, B.DEL_NOTE, B.MTRL_SHORT, B.REMARKS,
    B.REF_PERSON, B.EMPL_ID, C.REQ_GROUP_NAME, D.REQ_TYPE_NAME, E.ASSET_GROUP_NAME, F.ASSET_SUB_GROUP_NAME, G.BRAND_NAME, H.MODEL
    ...
    while($row = oci_fetch_assoc($res)){
      ...
      $dept_code = "SELECT a.*, d.DESCR as dept, b.LOCATION, b.EMPL_STATUS, c.DESCR from PERSON_TBL a, JobCur_ee b , 
            ...
            'PER_EMAIL' => $row1['PERSONELMAIL'], 'BUS_EMAIL' => $row1['BUSINESSMAIL'], 'REF_PERSON' => $row['REF_PERSON'], 
            ...
            'BRAND' => $row['BRAND'], 'MODEL_CODE' => $row['MODEL_CODE'], 'PRICE' => $row['UNIT_PRICE'], 'SER_NO1' => $row['SERIAL_NO1'], 
            ...
            'MTRL_SHORT' => $row['MTRL_SHORT'], 'WAR_START' => $row['WARRANTY_START_DATE'], 'WAR_MONTH' => $row['WARRANTY_MONTH'], 
