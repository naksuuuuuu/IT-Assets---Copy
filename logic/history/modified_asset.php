...
if (isset($_POST['data'])){
    ...
    // }
        ...
        while($row = oci_fetch_assoc($res)){
            ...
            $emp_id = "SELECT DISTINCT PO_NO FROM IT_ASSET_DETAILS WHERE EMPL_ID = :emp_id";
            $stmt = oci_parse(connection(), $emp_id);
            oci_bind_by_name($stmt, ':emp_id', $empId);
            oci_execute($stmt);
            ...
            while($row1 = oci_fetch_assoc($stmt)){
                if($row1['PO_NO'] == "" ){
                    ...
                    $head_sql = "SELECT DISTINCT A.DOC_NO, A.PO_NO, C.VENDOR_NAME, 
                                B.EMPL_ID, B.MTRL_SHORT, B.PO_ITEM, B.REQ_GRP_ID, E.REQ_GRP_NAME, B.REQ_TYPE_ID, D.REQ_TYPE_NAME, 
                                B.ASSET_GRP_CODE, F.ASSET_GRP_NAME, B.ASSET_SUB_GRP_CODE, G.ASSET_SUB_GRP_NAME, B.BRAND_CODE, H.BRAND_NAME, 
                                B.MODEL_CODE, I.MODEL_NAME
                                FROM IT_ASSET_HEADER A, IT_ASSET_DETAILS B, IT_ASSET_VENDORS C, IT_ASSET_REQ_TYPE D, IT_ASSET_REQ_GROUP E,
                    ...
                    $result.="<tr>
                           ...
                           <td>".$row2["VENDOR_NAME"]."</td>
                           <td>".$row["BUSINESSMAIL"]."</td>
                           <td hidden><input class='req_grp_code' value='".$row2["REQ_GRP_ID"]."'></td>
                           <td hidden><input class='req_grp_name' value='".$row2["REQ_GRP_NAME"]."'></td>
                           <td hidden><input class='req_type_code' value='".$row2["REQ_TYPE_ID"]."'></td>
                           <td hidden><input class='req_type_name' value='".$row2["REQ_TYPE_NAME"]."'></td>
                           <td hidden><input class='ass_grp_code' value='".$row2["ASSET_GRP_CODE"]."'></td>
                           <td hidden><input class='ass_grp_name' value='".$row2["ASSET_GRP_NAME"]."'></td>
                           <td hidden><input class='ass_sub_grp_code' value='".$row2["ASSET_SUB_GRP_CODE"]."'></td>
                           <td hidden><input class='ass_sub_grp_name' value='".$row2["ASSET_SUB_GRP_NAME"]."'></td>
                           <td hidden><input class='brand_code' value='".$row2["BRAND_CODE"]."'></td>
                           <td hidden><input class='brand_name' value='".$row2["BRAND_NAME"]."'></td>
                           <td hidden><input class='model_code' value='".$row2["MODEL_CODE"]."'></td>
                           <td hidden><input class='model_name' value='".$row2["MODEL_NAME"]."'></td>
                           <td hidden><input class='po_item' value=".$row2["PO_ITEM"]." hidden></td>
                           <td hidden><input class='doc_no1' value=".$row2["DOC_NO"]." hidden></td>
                        </tr>";
                }
            }
        }
